...
public class MovingGameObject extends GameObject 
{
	...
	public void move() 
	{
		...
		if (location.x > DragonScreen.screenWidth) {
			location.x -= DragonScreen.screenWidth;
		}
		if (location.x < -30) {
			location.x += DragonScreen.screenWidth;
		}

		if (location.y > DragonScreen.screenHeight) {
			location.y -= DragonScreen.screenHeight;
		}
		if (location.y < 0) {
			location.y += DragonScreen.screenHeight;
		}
